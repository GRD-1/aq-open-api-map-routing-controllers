...
describe("generateOpenAPISpec", () => {
  ...
  beforeEach(() => {
    ...
    sandbox
      .stub(require("routing-controllers"), "getMetadataArgsStorage")
      .returns(mockStorage);

    // Stub Reflect.getMetadata
    sandbox
      .stub(Reflect, "getMetadata")
      .callsFake((key: any, target: Object, propertyKey?: string | symbol) => {
  ...
  it("should generate OpenAPI spec with correct basic structure", () => {
    const config: IOpenAPIMapConfig = {
    ...
    assert.deepEqual(
      result.info,
      config.info,
      "Info object should match config"
    );
  ...
  it("should properly handle controller metadata and generate tags", () => {
    ...
    assert.equal(
      result.tags[0].name,
      "Test",
      "Tag name should match controller tag"
    );
    assert.equal(
      result.tags[0].description,
      "Test Controller",
      "Tag description should match controller description"
    );
  });
  ...
  it("should include security schemes in components", () => {
    ...
    const bearerAuth = result.components?.securitySchemes
      ?.bearerAuth as SecuritySchemeObject;
  ...
  it("should handle schema aliases correctly", () => {
    ...
    assert.isObject(
      result.components?.schemas?.TestResponseAlias,
      "Aliased response schema should be present"
    );
    assert.isObject(
      result.components?.schemas?.TestRequestAlias,
      "Aliased request schema should be present"
    );
  });
  ...
  it("should generate OpenAPI spec file in the specified location", () => {
    ...
    assert.isTrue(
      fs.existsSync(testMapPath),
      "Generated file should exist in the specified location"
    );
    ...
    assert.deepEqual(
      fileContent,
      result,
      "File content should match the returned spec"
    );

    // Verify file structure
    assert.isObject(
      fileContent,
      "Generated file should contain a valid JSON object"
    );
    assert.deepEqual(
      fileContent.info,
      config.info,
      "File should contain correct info object"
    );
