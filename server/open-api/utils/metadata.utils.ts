...
import { IOpenAPIMapConfig, IClass, IOpenAPISpec, IOperationMetadata } from '../types'
...
export function processControllerMetadata(controller: IClass, operation: IOperationMetadata, methodName: string): void {
...
export function updateControllerMetadata(controller: IClass, metadata: any, spec: IOpenAPISpec): void {
  ...
  Object.entries(spec.paths || {}).forEach(([, pathItem]) => {
    Object.entries(pathItem).forEach(([, operation]) => {
      if (operation.operationId?.startsWith(`${controller.name}.`)) {
        ...
        if (methodName) {
          processControllerMetadata(controller, operation as IOperationMetadata, methodName)
        }
      }
    })
  })
